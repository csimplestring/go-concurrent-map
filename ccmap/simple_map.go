...
func allocLinkedBuckets(length int) []Bucket {
...
// move moves 2 old entries to new bucket
func (l *LinkedMap) moveEntry(n int) {
...
func (l *LinkedMap) Put(key Key, val interface{}) error {
	...
	if len(l.newBuckets) == 0 {
		length := 2 * l.bucketSize
		l.newBuckets = allocLinkedBuckets(length)
